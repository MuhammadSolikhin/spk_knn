...
@section('content')
<h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">Master Data / Kriteria /</span> Tambah Baru</h4>
                ...
                <form action="{{ route('admin.kriteria.store') }}" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('kode') is-invalid @enderror" 
                            id="kode" name="kode" placeholder="Contoh: C1" value="{{ old('kode') }}" required />
                        @error('kode')
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('nama_kriteria') is-invalid @enderror" 
                            id="nama_kriteria" name="nama_kriteria" placeholder="Contoh: Nilai Matematika" value="{{ old('nama_kriteria') }}" required />
                        @error('nama_kriteria')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('tipe') is-invalid @enderror" id="tipe" name="tipe" required>
                            ...
                            <option value="core" {{ old('tipe') == 'core' ? 'selected' : '' }}>Core Factor (Utama)</option>
                            <option value="secondary" {{ old('tipe') == 'secondary' ? 'selected' : '' }}>Secondary Factor (Pendukung)</option>
                        </select>
                        @error('tipe')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
